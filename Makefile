# ===== FLEXT-QUALITY MAKEFILE - FLEXT STANDARD 2025 =====
# ==============================================================
...
_test-strict: ## Internal: Strict testing for quality gate
	...
	@if [ -d tests ]; then \
		PYTHONPATH=src $(PYTEST) tests/ --maxfail=1 --tb=no -q --cov-fail-under=90 || (echo "$(RED)$(CROSS) Tests failed$(NC)" && exit 1); \
...
test: ## $(MICROSCOPE) Run comprehensive test suite
	...
	@PYTHONPATH=src $(PYTEST) tests/ -v --tb=short --durations=10
...
test-coverage: ## $(MICROSCOPE) Run tests with detailed coverage
	...
	@PYTHONPATH=src $(PYTEST) tests/ -v --cov=src/$(PACKAGE_NAME) --cov-report=html --cov-report=term-missing --cov-fail-under=90
