...
class PackageDiscovery:
    ...
    def _analyze_package_fast(: self, dist: importlib.metadata.Distribution, ) -> dict[str, Any] | None:
        try:
            ...
            version,
                "location": location,
                "source_path":
                    location,  # Use location as source_path for now
                "package_type":
            package_type,
                "description": dist.metadata.get("Summary", ""),
                "has_python_files": True,  # Assume true to avoid file system checks
                "estimated_size":
                    0,  # Skip size estimation for performance
            }
        except Exception:
        return None

    def _analyze_package(: self, dist: importlib.metadata.Distribution, ) -> dict[str, Any] | None:
        try:
            name = dist.metadata["Name"]
            version = dist.metadata["Version"]
    ...
    def _determine_package_type(: self, dist: importlib.metadata.Distribution, location: str | None, ) -> str:
        if not location:
            return "system"
    ...
    def _get_package_location_fast(: self, dist: importlib.metadata.Distribution, ) -> str | None:
        try:
    ...
    def _determine_package_type_fast(: self, dist: importlib.metadata.Distribution, location: str | None, ) -> str:
        if not location:
            return "system"
    ...
    def _is_interesting_package(self, name: str) -> bool:
        # Always include custom/local packages (those with flx, client-b, etc.)
        if any(prefix in name.lower() for prefix in ["flx", "client-b"]):
            return True
        ...
        skip_packages = {"pip",
            ...
            "tomli",
            "typing-extensions",
            # "colorama", - removed prehistoric library
            "distlib",
            "filelock",
            "virtualenv",
            "pyproject-hooks",
    ...
    def _get_package_source_path(: self, package_name: str, location: str | None, ) -> str | None:
        if not location:
            return None
        ...
        possible_paths = [location_path / package_name,
    ...
    def get_analyzable_packages(self) -> list[dict[str, Any]]:
            packages = self.get_installed_packages()
        return [p for p in packages if p["has_python_files"] and p["estimated_size"] > 0:
        ]
