...
lint: ## Run the 6 lint gates
	...
	if [ -d "$(WORKSPACE_VENV)" ]; then \
		if [ -f "$(WORKSPACE_ROOT)/scripts/check/fix_pyrefly_config.py" ]; then \
			$(POETRY_ENV) python -m flext_infra check fix-pyrefly-config; \
		fi; \
		$(POETRY_ENV) python -m flext_infra check run --gates "$$gates" --reports-dir "$(CURDIR)/.reports/check" --projects "$(PROJECT_NAME)"; \
		exit $$?; \
	fi; \
...
mypy-check: ## Run type checking with Mypy
	$(VALIDATE_MYPY_LIMITS); $(MYPY_BOUNDED) $(POETRY) run mypy $(SRC_DIR) || { $(REPORT_MYPY_FAILURE); exit $$code; }
.PHONY: pyright-check
...
# CONFIGURATION
.DEFAULT_GOAL := help
.PHONY: help install-workspace install install-dev setup validate check lint format format-check type-check security fix dead-code modernize cognitive-complexity spell-check deps validate-full test test-unit test-integration test-quality test-django test-analysis test-e2e test-fast coverage-html build build-clean analyze quality-check metrics report workspace-analyze detect-issues calculate-scores quality-grade coverage-score web-start web-migrate web-shell web-collectstatic web-createsuperuser docs deps-update deps-show deps-audit shell pre-commit clean clean-all reset diagnose doctor
