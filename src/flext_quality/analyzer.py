...
class CodeAnalyzer:
    ...
    def analyze_project(self, 
        ...
        logger.info("Analysis completed. Files: %d, Lines: %d", 
                   results["files_analyzed"], results["total_lines"])
    ...
    def _find_python_files(self) -> list[Path]:
        ...
        for py_file in self.project_path.rglob("*.py"):
            ...
            if any(part.startswith(".") for part in py_file.parts):
                continue
            if any(ignore in str(py_file) for ignore in ["__pycache__", ".git", "node_modules"]):
                continue
    ...
    def _calculate_overall_metrics(self, file_metrics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def _analyze_complexity(self, file_metrics: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for py_file in self._find_python_files():
            try:
                ...
                for i, line in enumerate(lines):
                    if ("import " in line or "from " in line) and "# unused" in line.lower():
                        issues.append({
                            "file": str(py_file.relative_to(self.project_path)),
                            "line": i + 1,
                            "type": "unused_import",
