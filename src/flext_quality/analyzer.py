...
class FlextQualityAnalyzer:
    """Unified quality analyzer class following FLEXT architecture patterns.
    ...
    class FlextQualityCodeAnalyzer:
        ...
        def analyze_project(
            ...
            *,
            ...
            # Create issues list
            issues = self._collect_issues(file_metrics, include_duplicates)
            ...
            results = FlextQualityTypes.AnalysisResults(
                ...
                complexity_issues=[issue for issue in issues if issue.issue_type == "COMPLEXITY"],
                security_issues=[issue for issue in issues if issue.issue_type == "SECURITY"],
                dead_code_issues=[issue for issue in issues if issue.issue_type == "DEAD_CODE"],
                duplication_issues=[issue for issue in issues if issue.issue_type == "DUPLICATION"],
                dependencies=[],  # Would need dependency scanning
                test_results=None,  # Would need test execution
                analysis_config=self._config.get_analysis_config(),
                analysis_timestamp=None,  # Will be set by Pydantic
            )

            self._current_results = results
            logger.info("Analysis completed for project: %s", self.project_path)
            return results
        ...
        def _analyze_file(self, file_path: Path) -> FlextResult[FlextQualityTypes.FileAnalysisResult]:
            ...
            try:
                ...
                style_issues = self._detect_style_issues(tree)
                dead_code_lines = self._detect_dead_code(tree)

                return FlextResult.ok(FlextQualityTypes.FileAnalysisResult(
                    file_path=file_path,
        ...
        def _detect_style_issues(self, tree: ast.AST) -> int:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    if len(node.name) > 50:  # Long function names
                        issues += 1
            return issues
        ...
        def _collect_issues(
            ...
            for metrics in file_metrics:
                # Add security issues
                for i in range(metrics.security_issues):
                    issues.append(FlextQualityTypes.CodeIssue(
                ...
                if metrics.complexity_score > self._config.max_complexity:
                    issues.append(FlextQualityTypes.CodeIssue(
                        file_path=str(metrics.file_path),
                        line_number=1,  # Simplified
            ...
            for py_file in python_files:
                try:
                    with py_file.open(encoding="utf-8") as f:
                        content = f.read()
                    if len(content.strip()) > FlextQualityAnalyzer.AnalysisConstants.MIN_FILE_SIZE_FOR_DUPLICATION_CHECK:
        ...
        def _files_have_duplication(self, content1: str, content2: str) -> bool:
            ...
            if lines1 and lines2:
                ...
                return similarity > FlextQualityAnalyzer.AnalysisConstants.SIMILARITY_THRESHOLD

            return False


# Legacy compatibility alias
CodeAnalyzer = FlextQualityAnalyzer.FlextQualityCodeAnalyzer